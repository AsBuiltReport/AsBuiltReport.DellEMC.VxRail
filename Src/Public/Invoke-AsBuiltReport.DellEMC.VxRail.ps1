function Invoke-AsBuiltReport.DellEMC.VxRail {
    ...
    foreach ($VIServer in $Target) {
        ...
        $VxmSystem = Get-VxRailApi -Uri '/system'
        $VxmProxy = Get-VxRailApi -Uri  '/system/proxy'
        $VxmCallHomeMode = Get-VxRailApi -Uri  '/callhome/mode'
        $VxmCallHomeInfo = Get-VxRailApi -Uri '/callhome/info'
        $VxmInternetMode = Get-VxRailApi -Uri '/system/internet-mode'
        $VxmAvailableHosts = Get-VxRailApi -Uri '/system/available-hosts'
        $VxmClusterHosts = Get-VxRailApi -Uri '/system/cluster-hosts'
        ...
        Section -Style Heading1 "$($VxRailMgr.Name)" {
            ...
            if ($InfoLevel.Cluster -gt 0) {
                Section -Style Heading2 'Cluster' {
                    ...
                    $TableParams = @{
                        Name = "VxRail Cluster - $($VxRailMgr.Name)"
                        List = $true
                        ColumnWidths = 50, 50
                    }
                    ...
                    Section -Style Heading3 'Installed Components' {
                        ...
                        $TableParams = @{
                            Name = "Installed Components - $($VxRailMgr.Name)"
            ...
            Write-PScriboMessage "Appliances InfoLevel set at $($InfoLevel.Appliances)."
            if ($InfoLevel.Appliances -gt 0) {
                if ($VxmClusterHosts) {
                        ...
                        foreach ($VxmClusterHost in $VxmClusterHosts) {
                            Section -Style Heading3 "$($VxmClusterHost.host_name)" {
                                ...
                                Section -Style Heading4 "Hardware" {
                                    ...
                                    $TableParams = @{
                                        Name = "Hardware Information - $($VxmClusterHost.host_name)"
                                        List = $true
                                        ColumnWidths = 50, 50
                                    }
                                    if ($Report.ShowTableCaptions) {
                                    ...
                                    $ClusterHosts | Table @TableParams
                                }
                                ...
                                Section -Style Heading4 'ESXi' {
                                    ...
                                    $TableParams = @{
                                        Name = "ESXi Information - $($VxmClusterHost.host_name)"
                                        List = $true
                                        ColumnWidths = 50, 50
                                    }
                                    if ($Report.ShowTableCaptions) {
                                ...
                                $VxmClusterHostiDRAC = Get-VxRailApi -Uri ('/hosts/' + $VxmClusterHost.serial_number + '/idrac/network')
                                Section -Style Heading4 'iDRAC' {
                                    ...
                                    $TableParams = @{
                                        Name = "iDRAC Information - $($VxmClusterHost.host_name)"
                                        List = $true
                                        ColumnWidths = 50, 50
                                    }
                                    if ($Report.ShowTableCaptions) {
            ...
            if ($VxmCallHome -and $VxmCallHomeInfo) {
                Section -Style Heading2 'Support' {
                    Section -Style Heading3 'Dell EMC Secure Remote Service (ESRS)' {
                        ...
                        $TableParams = @{
                            Name = "Dell EMC Secure Remote Service - $($VxRailMgr.Name)"
                            List = $true
                            ColumnWidths = 50, 50
                        }
                        if ($Report.ShowTableCaptions) {
            ...
            if ($InfoLevel.Network -gt 0) {
                Section -Style Heading2 'Networking' {
                    ...
                    $TableParams = @{
                        Name = "Networking - $($VxRailMgr.Name)"
                        List = $true
                        ColumnWidths = 50, 50
                    }
                    if ($Report.ShowTableCaptions) {
                    ...
                    if ($VxmProxy) {
                        Section -Style Heading3 'Proxy Server' {
                            ...
                            $TableParams = @{
                                Name = "Proxy Server - $($VxRailMgr.Name)"
                                List = $true
                                ColumnWidths = 50, 50
                            }
                            if ($Report.ShowTableCaptions) {
