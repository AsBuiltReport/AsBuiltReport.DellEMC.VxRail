function Get-VxRailApi {
    ...
    Begin {
    #region Workaround for SelfSigned Cert an force TLS 1.2 (Windows PowerShell only)
    # ServicePointManager is obsolete on PowerShell 7/.NET Core (SYSLIB0014) and is not
    # needed there - self-signed certs are instead handled via -SkipCertificateCheck
    # on Invoke-RestMethod in the Process block below.
